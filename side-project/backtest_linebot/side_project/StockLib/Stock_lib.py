...
def draw_table_1(stock, profit, loss, recent_date, buy_date, buy_price, sell_date, sell_price, ROI):
    ...
    # 總損益 = 每次交易的錢相加
    total_profit_and_loss = sum(profit) + sum(loss)
    table_1_content.append(str("{:.2f}".format(total_profit_and_loss*1000)))
    print("總損益：%f" % (total_profit_and_loss))

    # 總獲利 = 每次賺錢的錢相加
    total_profit = sum(profit)
    table_1_content.append(str("{:.2f}".format(total_profit*1000)))
    print("總獲利：%f" % (total_profit))

    # 總虧損 = 每次賠錢的錢相加
    total_loss = sum(loss)
    table_1_content.append(str("{:.2f}".format(total_loss*1000)))
    ...
    table_1_content.append(str("{:.2f}".format(max_profit*1000)))
    print("最大交易獲利：%f" % (max_profit))

    # 最大交易虧損
    max_loss = min(loss)
    table_1_content.append(str("{:.2f}".format(max_loss*1000)))
    ...
    if len(sell_date) == len(buy_date):
        ...
        unrealized_gain_and_loss =  ((last_close_price - (buy_price[-1]*1.004425) - (last_close_price*0.001425)))
        table_1_content.append(str("{:.2f}".format(unrealized_gain_and_loss*1000)))
...
def darw_table_2(stock, recent_date, buy_date, buy_price, sell_date, sell_price, ROI):
    ...
    for i in range(len(sell_price)):
        ...
        inside_content.append("{:.2f}".format(sell_price[i]*0.003*1000)) #證交稅
...
def pics_combine(table_1, kline, table_2, strategy_name, stock_name):
    final_result = Page()
    final_result.add(table_1)
    final_result.add(kline)
    final_result.add(table_2)
    final_result.render("%s_%s策略回測.html" % (stock_name, strategy_name))
